...
export class DiagramDiffTracker {
    ...
    private generateSummary(
        ...
        if (modified.length > 0) {
            const textChanges = modified.filter(
                (m) => m.after.value !== undefined,
            )
            const positionChanges = modified.filter(
                (m) => m.after.geometry !== undefined,
            )
            const styleChanges = modified.filter(
                (m) => m.after.style !== undefined,
            )
...
export function diffToAgentFormat(diff: DiagramDiff): string {
    ...
    if (diff.added.length > 0) {
        ...
        for (const cell of diff.added) {
            if (cell.vertex) {
                lines.push(
                    `- Node "${cell.value || "unnamed"}" (id: ${cell.id})`,
                )
            } else if (cell.edge) {
    ...
    if (diff.modified.length > 0) {
        ...
        for (const mod of diff.modified) {
            ...
            if (mod.after.value !== undefined) {
                changes.push(
                    `text: "${mod.before.value}" → "${mod.after.value}"`,
                )
            }
