...
/** Build one isolated MCP SDK 2 server instance for a single client connection. */
    ...
    server.registerTool(
        ...
        async () => {
            try {
                ...
                log.info(
                    `Started session ${sessionId}, browser at ${browserUrl}`,
                )
    ...
    server.registerTool(
        ...
        async ({ operations }) => {
            try {
                ...
                const timeSinceGet =
                    Date.now() - currentSession.lastGetDiagramTime
                ...
                log.info(
                    `Editing diagram with ${operations.length} operation(s)`,
                )
                ...
                if (errors.length > 0) {
                    const errorMessages = errors
                        ...
                        .join("\n")
                    log.warn(
                        `Edit had ${errors.length} error(s): ${errorMessages}`,
                    )
                }
        ...
        async () => {
            try {
                ...
                if (syncRequested) {
                    ...
                    if (!synced) {
                        log.warn(
                            "get_diagram: sync timeout - state may be stale",
                        )
                    }
                }
...
let closeStdio: (() => Promise<void>) | null = null
...
function installShutdownHandlers() {
    ...
    process.stdin.on("close", () => void gracefulShutdown("stdin closed"))
    process.stdin.on("end", () => void gracefulShutdown("stdin ended"))
    ...
    process.stdout.on("error", (err) => {
        if (err.code === "EPIPE" || err.code === "ERR_STREAM_DESTROYED") {
            void gracefulShutdown("stdout error")
        }
    })
}
