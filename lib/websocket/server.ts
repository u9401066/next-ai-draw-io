...
function handleClientMessage(clientId: string, message: WSMessage): void {
    ...
    switch (message.type) {
        ...
        case "subscribe": {
            ...
            console.log(
                `[WS] Client ${clientId} subscribed to tab: ${subscribePayload.tabId}`,
            )
            break
        }
...
let changesReportHandler:
    | ((clientId: string, message: WSMessage) => void)
    | null = null
let operationResultHandler:
    | ((clientId: string, message: WSMessage) => void)
    | null = null

export function setChangesReportHandler(
    handler: (clientId: string, message: WSMessage) => void,
): void {
    changesReportHandler = handler
}

export function setOperationResultHandler(
    handler: (clientId: string, message: WSMessage) => void,
): void {
