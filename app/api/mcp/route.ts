...
async function forwardToWsServer(
    action: string,
    data?: any,
...
// === Diff 相關狀態 ===
// 儲存基準 XML（用於計算 diff）
const baseXmlState: { [tabId: string]: string } = {}
// 儲存用戶變更摘要（由瀏覽器定期回報）
let humanChanges: {
    hasChanges: boolean
    operations: {
...
export async function GET(req: NextRequest) {
    ...
    if (action === "get_export_result") {
        ...
        if (!exportRequest || exportRequest.requestId !== requestId) {
            return NextResponse.json(
                { error: "No matching export request" },
                { status: 404 },
            )
        }
    ...
    if (action === "clear_events") {
        ...
        const before = parseInt(
            searchParams.get("before") || String(Date.now()),
        )
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (action === "display") {
            // 嘗試透過 WebSocket 推送
            const wsResponse = await forwardToWsServer("display", {
                xml,
                tabId,
                tabName,
            })
        ...
        if (action === "user_save" || action === "user_autosave") {
            ...
            const targetTabName =
                tabName ||
                tabs.find((t) => t.id === targetTabId)?.name ||
                "Untitled"
            ...
            console.log(
                `[MCP] User ${eventType} event received for tab: ${targetTabName}`,
            )
        ...
        if (action === "export_result") {
            ...
            if (exportRequest && exportRequest.requestId === requestId) {
                ...
                return NextResponse.json({
                    success: true,
                    message: "Export result received",
                })
            }
            return NextResponse.json(
                { error: "No matching export request" },
