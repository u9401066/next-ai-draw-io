...
const NODE_STYLES: Record<string, string> = {
    ...
    hexagon:
        "shape=hexagon;perimeter=hexagonPerimeter2;whiteSpace=wrap;html=1;fillColor=#f8cecc;strokeColor=#b85450;",
    document:
        "shape=document;whiteSpace=wrap;html=1;fillColor=#fff2cc;strokeColor=#d6b656;",
...
export class DiagramOperationsHandler {
            ...
            modified: diff.modified.flatMap((mod) => {
                ...
                if (mod.after.value !== undefined) {
                    changes.push({
                        id: mod.id,
                        field: "value",
                        before: mod.before.value,
                        after: mod.after.value,
                    })
                }
                if (mod.after.geometry !== undefined) {
                    changes.push({
                        id: mod.id,
                        field: "position",
                        before: mod.before.geometry,
                        after: mod.after.geometry,
                    })
                }
                if (mod.after.style !== undefined) {
                    changes.push({
                        id: mod.id,
                        field: "style",
                        before: mod.before.style,
                        after: mod.after.style,
                    })
                }
                return changes
            }),
        ...
        for (let i = 0; i < cells.length; i++) {
            ...
            if (isVertex) {
                ...
                if (geometry) {
                    ...
                    element.size = {
                        width: parseFloat(
                            geometry.getAttribute("width") || "0",
                        ),
                        height: parseFloat(
                            geometry.getAttribute("height") || "0",
                        ),
                    }
                }
            }
    ...
    private hasConflict(
        op: DiagramOperation,
        userChanges: DiagramDiff,
    ): boolean {
        if (op.op === "modify_node" || op.op === "delete_node") {
            // 檢查用戶是否也修改了這個節點
            const userModified = userChanges.modified.find(
                (m) => m.id === op.id,
            )
